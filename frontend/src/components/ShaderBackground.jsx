...
// UNIFORMS adaptados a la estetica "Red Underground" del flyer de referencia.
//
// Decision de diseño: en el flyer el campo es NEGRO y el rojo neon aparece solo
// como acento (los tubos). Por eso la rampa de color se queda en humo rojo
// oscuro y nunca llega al neon puro: el neon lo aportan los bordes y glows de
// la UI. Un fondo de plasma rojo saturado se come el texto blanco.
const UNIFORMS = {
  colors: [
    [0.02, 0.0, 0.004], // #050001 negro humo
    [0.078, 0.0, 0.012], // #140003 humo tenue
    [0.239, 0.0, 0.035], // #3d0009 humo rojo
    [0.478, 0.0, 0.078], // #7a0014 rojo sangre (tope de la rampa)
    [0.478, 0.0, 0.078],
    [0.478, 0.0, 0.078],
    [0.478, 0.0, 0.078],
    [0.478, 0.0, 0.078],
  ...
  intensity: 0.5,
  paramA: 0.5,
  warp: 0.35, // deforma el patron para leerse como humo
  detail: 2.4,
  contrast: 1.1,
  brightness: -0.02,
  saturation: 1.15,
  hue: 0.0,
  vignette: 0.55, // oscurece los bordes, como en el flyer
  blur: 0.0,
  grain: 0.05,
  seed: 3.0,
  rotate: 0.0,
  ...
  cursorStrength: 1.0,
  cursorRadius: 0.202,
  oklab: 1.0, // interpolar en OkLab evita rojos "sucios"
  timeScale: 0.38,
};
...
export default function ShaderBackground() {
  ...
  useEffect(() => {
    ...
    if (!gl) return; // sin WebGL queda el degradado CSS de body::before
    ...
    const compile = (type, src) => {
      ...
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        console.error('[shader] compilacion fallida:', gl.getShaderInfoLog(shader));
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error('[shader] enlazado fallido:', gl.getProgramInfoLog(program));
    ...
    // Con "reducir movimiento" activo dejamos un fotograma estatico.
  ...
  return (
    ...
    >
      ...
      />
      {/* Scrim: garantiza contraste del texto sin importar como caiga el
          patron del shader en cada resolucion. */}
      <div
        className="absolute inset-0"
        style={{
          background:
            'radial-gradient(120% 90% at 50% 30%, rgba(4,0,1,0.45) 0%, rgba(4,0,1,0.7) 55%, rgba(4,0,1,0.9) 100%)',
        }}
      />
    </div>
  );
